...
.status-merging { background: #dbeafe; color: #1e40af; }
...
"#,
    ...
    if queued.is_empty() {
        ...
        html.push_str(
            "<table><thead><tr><th>PR</th><th>Repo</th><th>Author</th><th>Status</th><th>Queued At</th></tr></thead><tbody>",
        );
        for pr in &queued {
            html.push_str(&format!(
                "<tr><td class=\"mono\">#{}</td><td class=\"mono\">{}/{}</td><td>{}</td><td><span class=\"status status-{}\">{}</span></td><td class=\"mono\">{}</td></tr>",
                pr.pr_number,
                pr.repo_owner,
                pr.repo_name,
                pr.author,
                pr.status,
                pr.status,
                pr.queued_at.map(|t| t.format("%Y-%m-%d %H:%M:%S").to_string()).unwrap_or_default(),
            ));
        }
